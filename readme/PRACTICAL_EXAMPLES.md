...
class CustomDetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        anchor_points, stride_tensor = self._get_coalesced_xyxy(batch_size, imgsz, self.stride[0], self.device, dtype)
        ...
        for i in range(batch_size):
            target_scores, target_bboxes, fg_mask, _ = self.assigner(
                ...
                (torch.ones(1, *target_scores.shape[1:]) if len(gt_bboxes[i]) > 0 else torch.zeros(1, 0, 1)),
            )
...
class ResearchVisualizer:
    ...
    def plot_architecture_comparison(results_json):
        ...
        for i, name in enumerate(names):
            axes[1, 1].annotate(name, (params[i], mAP_values[i]), ha="center", va="bottom")
