...
def create_3panel_figure(data):
    ...
    ax2.axhline(
        y=baseline_mAP50, color="black", linestyle="--", linewidth=2.5, label="Baseline (No SAHI)", alpha=0.8, zorder=1
    )
    ...
    print(
        f"   Best config FPS: {data['fps'][best_mAP_idx]:.2f} ({((data['fps'][best_mAP_idx] / data['fps'][0]) - 1) * 100:.1f}% vs baseline)"
    )
