...
def create_compact_figure():
    ...
    ax1.axvline(
        np.mean(widths), color="red", linestyle="--", linewidth=2.5, label=f"Mean Width: {np.mean(widths):.1f}px"
    )
    ax1.axhline(
        np.mean(heights), color="green", linestyle="--", linewidth=2.5, label=f"Mean Height: {np.mean(heights):.1f}px"
    )
    ax1.set_xlabel("Width (pixels)", fontsize=13, fontweight="bold")
    ax1.set_ylabel("Height (pixels)", fontsize=13, fontweight="bold")
    ax1.set_title("(A) Bounding Box Dimensions: Width vs Height", fontsize=14, fontweight="bold", loc="left", pad=10)
    ...
    ax2.axvline(np.mean(widths), color="red", linestyle="--", linewidth=2.5, label=f"Mean: {np.mean(widths):.1f}px")
    ax2.axvline(
        np.median(widths), color="green", linestyle="--", linewidth=2.5, label=f"Median: {np.median(widths):.1f}px"
    )
    ...
    ax3.axvline(np.mean(heights), color="red", linestyle="--", linewidth=2.5, label=f"Mean: {np.mean(heights):.1f}px")
    ax3.axvline(
        np.median(heights), color="green", linestyle="--", linewidth=2.5, label=f"Median: {np.median(heights):.1f}px"
    )
    ...
    # Overall title
    fig.suptitle(
        "VisDrone Person Class (Pedestrian + People): Dimension Analysis", fontsize=16, fontweight="bold", y=0.98
    )
