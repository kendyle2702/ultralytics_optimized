...
def create_publication_figures(data: dict, results: dict[str, dict]):
    ...
    for i, overlap in enumerate(unique_overlaps):
        for j, slice_size in enumerate(unique_slices):
            idx = [
                (k, s, o, m)
                for k, (s, o, m) in enumerate(zip(slice_sizes_sahi, overlaps_sahi, mAP50_sahi))
    ...
    sns.heatmap(
        ...
        fmt=".3f",
        cmap="YlOrRd",
        xticklabels=unique_slices,
        yticklabels=unique_overlaps,
    ...
    ax2.axhline(y=baseline_mAP50, color="black", linestyle="--", linewidth=2, label="Baseline (No SAHI)", alpha=0.7)
    ...
    for i, overlap in enumerate(unique_overlaps):
        ...
        ax2.plot(slices, mAPs, marker="o", linewidth=2, markersize=6, label=f"Overlap={overlap}", color=colors[i])
    ...
    for i, overlap in enumerate(unique_overlaps):
        ...
        for fps, mAP, slc in zip(fps_vals, mAP50_vals, slice_vals):
            if overlap == 0.2:  # Only label overlap=0.2 to avoid clutter
                ax3.annotate(str(slc), (fps, mAP), fontsize=7, xytext=(3, 3), textcoords="offset points")
    ...
    ax5.axhline(y=baseline_mAPs, color="black", linestyle="--", linewidth=2, label="Baseline", alpha=0.7)

    for i, overlap in enumerate(unique_overlaps):
        indices = [k for k, (s, o) in enumerate(zip(slice_sizes_sahi, overlaps_sahi)) if o == overlap]
        slices = [slice_sizes_sahi[k] for k in indices]
        mAPs_vals = [
            data["mAPs"][sahi_indices[k]]
            for k in range(len(sahi_indices))
            if slice_sizes_sahi[k] in slices and overlaps_sahi[k] == overlap
        ]
        slices_sorted = sorted(
            set([slice_sizes_sahi[k] for k in range(len(sahi_indices)) if overlaps_sahi[k] == overlap])
        )

        # Get corresponding mAPs values
        mAPs_sorted = []
        for slc in slices_sorted:
            idx = [k for k in range(len(sahi_indices)) if slice_sizes_sahi[k] == slc and overlaps_sahi[k] == overlap]
            if idx:
    ...
    table_data.append(
        [
            "Best Accuracy",
            f"+{acc_improve_mAP50:.0f}%",
            f"+{acc_improve_mAPs:.0f}%",
    ...
    table_data.append(
        [
            "Best Balanced",
            f"+{bal_improve_mAP50:.0f}%",
            f"+{bal_improve_mAPs:.0f}%",
            f"{((data['fps'][best_balanced_idx] / data['fps'][0]) - 1) * 100:.0f}%",
        ]
    )

    table = ax6.table(cellText=table_data, cellLoc="center", loc="center", colWidths=[0.35, 0.2, 0.2, 0.2])
    ...
    for i, overlap in enumerate(unique_overlaps):
        ...
        sorted_data = sorted(zip(slices, mAPs))
        slices, mAPs = zip(*sorted_data)
        ax2.plot(slices, mAPs, marker="o", linewidth=2, markersize=7, label=f"Overlap={overlap}", color=colors[i])
    ax2.set_xlabel("Slice Size (pixels)")
    ax2.set_ylabel("mAP50")
...
def main():
    ...
    input("Press ENTER to start grid search (or Ctrl+C to cancel)... ")
    ...
    print(
        f"   mAP50: {data['mAP50'][best_acc_idx]:.4f} (+{((data['mAP50'][best_acc_idx] / data['mAP50'][0]) - 1) * 100:.0f}%)"
    )
    print(
        f"   mAPs:  {data['mAPs'][best_acc_idx]:.4f} (+{((data['mAPs'][best_acc_idx] / data['mAPs'][0]) - 1) * 100:.0f}% if data['mAPs'][0] > 0 else 'N/A')"
    )
    ...
    print(
        f"   mAP50: {data['mAP50'][best_bal_idx]:.4f} (+{((data['mAP50'][best_bal_idx] / data['mAP50'][0]) - 1) * 100:.0f}%)"
    )
    print(
        f"   mAPs:  {data['mAPs'][best_bal_idx]:.4f} (+{((data['mAPs'][best_bal_idx] / data['mAPs'][0]) - 1) * 100:.0f}% if data['mAPs'][0] > 0 else 'N/A')"
    )
