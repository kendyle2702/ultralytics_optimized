...
def draw_boxes(image, boxes, scores, color=(0, 255, 0)):
    ...
    for box, score in zip(boxes, scores):
        ...
        cv2.putText(img_draw, f"{score:.2f}", (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    return img_draw


def run_baseline(image_path, model_path):
    """Run baseline YOLO inference."""
    print("🔄 Running baseline YOLO...")

    # Load model
    model = YOLO(model_path)

    # Run inference
    results = model.predict(str(image_path), imgsz=640, conf=CONF_THRESHOLD, iou=IOU_THRESHOLD, verbose=False)
...
def main():
    ...
    parser.add_argument("--model", "-m", type=str, default=MODEL_PATH, help="Model path (default: v8_trained.pt)")
    parser.add_argument("--output", "-o", type=str, default=None, help="Output directory (default: comparison_outputs)")
    ...
    print(f"\n{'=' * 60}")
    print("🎯 Comparing YOLO Baseline vs SAHI")
    print(f"{'=' * 60}")
    ...
    cv2.putText(img_comparison, label_baseline, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
    cv2.putText(img_comparison, label_sahi, (w + 20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 3)

    cv2.putText(
        img_comparison, f"Detections: {len(boxes_baseline)}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
    )
    cv2.putText(
        img_comparison, f"Detections: {len(boxes_sahi)}", (w + 20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2
    )
