...
def analyze_person_class(all_bboxes):
    ...
    print(f"\n{'=' * 80}")
    print("📊 PHÂN TÍCH CLASS PERSON (Pedestrian + People)")
    print(f"{'=' * 80}")
...
def plot_visualizations(df, save_dir="visdrone_analysis"):
    ...
    _hist, _xedges, _yedges = np.histogram2d(sample_widths, sample_heights, bins=50)

    ax.scatter(sample_widths, sample_heights, c="#3498db", alpha=0.3, s=10, edgecolors="none")

    # Add mean lines
    ax.axvline(np.mean(widths), color="red", linestyle="--", linewidth=2, label=f"Mean Width: {np.mean(widths):.1f}px")
    ax.axhline(
        np.mean(heights), color="green", linestyle="--", linewidth=2, label=f"Mean Height: {np.mean(heights):.1f}px"
    )

    ax.set_xlabel("Width (pixels)", fontsize=12, fontweight="bold")
    ax.set_ylabel("Height (pixels)", fontsize=12, fontweight="bold")
    ax.set_title(
        "VisDrone Person Class: Bounding Box Dimensions Distribution\n(Pedestrian + People merged)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    ax.set_ylabel("Height (pixels)", fontsize=12, fontweight="bold")
    ax.set_title(
        "VisDrone Person Class: Bounding Box Density Heatmap\n(Pedestrian + People merged)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    axes[0].axvline(np.mean(widths), color="red", linestyle="--", linewidth=2, label=f"Mean: {np.mean(widths):.1f}px")
    axes[0].axvline(
        np.median(widths), color="green", linestyle="--", linewidth=2, label=f"Median: {np.median(widths):.1f}px"
    )
    axes[0].set_xlabel("Width (pixels)", fontsize=12, fontweight="bold")
    axes[0].set_ylabel("Frequency", fontsize=12, fontweight="bold")
    axes[0].set_title("Width Distribution of Person Bounding Boxes", fontsize=13, fontweight="bold")
    axes[0].legend(fontsize=10)
    axes[0].grid(True, alpha=0.3, linestyle="--")

    # Height distribution
    axes[1].hist(heights, bins=100, color="#e74c3c", alpha=0.7, edgecolor="black")
    axes[1].axvline(np.mean(heights), color="red", linestyle="--", linewidth=2, label=f"Mean: {np.mean(heights):.1f}px")
    axes[1].axvline(
        np.median(heights), color="green", linestyle="--", linewidth=2, label=f"Median: {np.median(heights):.1f}px"
    )
    axes[1].set_xlabel("Height (pixels)", fontsize=12, fontweight="bold")
    axes[1].set_ylabel("Frequency", fontsize=12, fontweight="bold")
    axes[1].set_title("Height Distribution of Person Bounding Boxes", fontsize=13, fontweight="bold")
    ...
    ax.set_title(
        "VisDrone Person Class: Statistical Summary of Dimensions\n(Pedestrian + People merged)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    # Prepare data
    pd.DataFrame(
        {"Dimension": ["Width"] * len(widths) + ["Height"] * len(heights), "Size": np.concatenate([widths, heights])}
    )

    violin = ax.violinplot([widths, heights], positions=[1, 2], showmeans=True, showmedians=True)
    ...
    ax.set_ylabel("Size (pixels)", fontsize=12, fontweight="bold")
    ax.set_title(
        "VisDrone Person Class: Distribution Shape of Dimensions\n(Pedestrian + People merged)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    ax1.axvline(np.mean(widths), color="red", linestyle="--", linewidth=2, label=f"Mean Width: {np.mean(widths):.1f}px")
    ax1.axhline(
        np.mean(heights), color="green", linestyle="--", linewidth=2, label=f"Mean Height: {np.mean(heights):.1f}px"
    )
    ...
    ax2.axvline(np.mean(widths), color="red", linestyle="--", linewidth=2, label=f"Mean: {np.mean(widths):.1f}px")
    ax2.axvline(
        np.median(widths), color="green", linestyle="--", linewidth=2, label=f"Median: {np.median(widths):.1f}px"
    )
    ...
    ax3.axvline(np.mean(heights), color="red", linestyle="--", linewidth=2, label=f"Mean: {np.mean(heights):.1f}px")
    ax3.axvline(
        np.median(heights), color="green", linestyle="--", linewidth=2, label=f"Median: {np.median(heights):.1f}px"
    )
    ...
    table = ax5.table(cellText=stats_data, cellLoc="center", loc="center", colWidths=[0.3, 0.35, 0.35])
    ...
    # Overall title
    fig.suptitle(
        "VisDrone Person Class (Pedestrian + People): Comprehensive Dimension Analysis",
        fontsize=16,
        fontweight="bold",
        y=0.995,
    )
