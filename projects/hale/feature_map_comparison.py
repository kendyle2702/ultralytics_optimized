...
print("💾 Input image saved: 00_input_image.jpg")
...
for idx, (name, layer_info) in enumerate(CBAM_LAYERS.items(), len(LAYERS_TO_COMPARE) + 1):
    ...
    for method in ["mean", "max", "grid"]:
        ...
        ax.set_title(f"{name} - {layer_info['description']}\nMethod: {method.upper()}", fontsize=14, fontweight="bold")
        ax.axis("off")
...
for stat in statistics_comparison:
    ...
    print(
        f"{stat['layer']:<20} {'Channel Variance':<25} {stat['base_variance']:<15.6f} "
    ...
    print(
        f"{'':20} {'Sparsity':<25} {stat['base_sparsity']:<15.6f} "
...
print("   - More diverse features → Better discriminative capability")

print("\n2. CBAM ATTENTION:")
print(f"   - CBAM layers {CBAM_LAYERS['CBAM_1']['opt']} and {CBAM_LAYERS['CBAM_2']['opt']} show focused attention")
print("   - Visualizations demonstrate spatial and channel-wise attention")

print("\n3. SCDOWN vs CONV:")
neck_stats = [s for s in statistics_comparison if "Neck" in s["layer"]]
if neck_stats:
    print("   - SCDown (layer 20) produces richer features than standard Conv (layer 16)")
    print("   - Evidence: Higher channel variance in neck layers")

print("\n📁 OUTPUT STRUCTURE:")
print("   - Input image: 00_input_image.jpg")
print(f"   - Layer comparisons: 01-{len(LAYERS_TO_COMPARE):02d}_*.jpg")
print(
    f"   - CBAM visualizations: {len(LAYERS_TO_COMPARE) + 1:02d}-{len(LAYERS_TO_COMPARE) + len(CBAM_LAYERS):02d}_*.jpg"
)
print("   - Each layer: 3 visualization methods (mean, max, grid)")
