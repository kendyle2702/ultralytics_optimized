...
def main():
    ...
    results = model.val(
        data=dataset_yaml, split=split, imgsz=640, batch=16, conf=0.001, iou=0.6, device="0", verbose=True
    )
    ...
    # Tính FPS
    total_time_ms = metrics["speed_preprocess_ms"] + metrics["speed_inference_ms"] + metrics["speed_postprocess_ms"]
