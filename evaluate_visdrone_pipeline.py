...
class VisDroneEvaluator:
    ...
    def __init__(
        ...
        if self.verbose:
            print(f"\n{'=' * 80}")
            print("🚀 VisDrone Evaluation Pipeline")
            print(f"{'=' * 80}")
    ...
    def _load_ground_truth(self, image_file: Path) -> list[dict]:
        ...
        with open(label_file) as f:
            for line in f:
                ...
                ground_truths.append({"class": cls, "bbox": [x1, y1, x2, y2], "area": width * height})
    ...
    def _calculate_coco_metrics(self, ground_truths: list[dict], detections: list[dict]) -> dict:
        ...
        coco_gt = {"images": [], "annotations": [], "categories": [{"id": 0, "name": "person"}]}
...
def main():
    ...
    parser.add_argument(
        "--split", type=str, default="val", choices=["train", "val", "test"], help="Dataset split to evaluate on"
    )

    # SAHI arguments
    parser.add_argument("--sahi", action="store_true", help="Use SAHI for sliced inference")
    parser.add_argument(
        "--no-sahi", dest="sahi", action="store_false", help="Do NOT use SAHI (standard YOLO inference)"
    )
    parser.set_defaults(sahi=False)

    parser.add_argument("--slice-size", type=int, default=640, help="Slice size for SAHI (height = width)")
    parser.add_argument("--overlap", type=float, default=0.2, help="Overlap ratio for SAHI (0.0 - 1.0)")

    # Inference arguments
    parser.add_argument("--imgsz", type=int, default=640, help="Image size for standard YOLO inference")
    parser.add_argument("--conf", type=float, default=0.1, help="Confidence threshold")
    parser.add_argument("--iou", type=float, default=0.5, help="IoU threshold for NMS")
    parser.add_argument("--device", type=str, default="cuda", help="Device (cpu, cuda, 0, 1, etc.)")

    # Output arguments
    parser.add_argument(
        "--output", type=str, default="results/evaluation_results.json", help="Output file for results (JSON)"
    )
